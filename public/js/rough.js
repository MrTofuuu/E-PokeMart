...
var nearbyButtonEl = document.getElementById("nearbyBtn");
var byCityButtonEl = document.getElementById("byZipBtn");
        ...
        if (response.ok) {
          response.json().then(function (data) {
            ...
            for (i = 0; i < data.length; i++) {
              ...
              setCardInfoNearby(i, city, name, phone, postalCode, state, street, website);
...
function setCardInfoNearby(index, city, name, phone, postalCode, state, street, website) {
  cardsEl;
  var cardContainer = document.createElement("div");
  var cardContainerClassAttr = document.createAttribute("class");
  cardContainerClassAttr.value = "col s12 m7";
  cardContainer.setAttributeNode(cardContainerClassAttr);
  var cardContainerId = document.createAttribute("id");
  cardContainerId.value = "card-container";
  cardContainer.setAttributeNode(cardContainerId)
  cardsEl.appendChild(cardContainer);
  ...
  cardAddrIno3.textContent = formatPhoneNumber(phone);
...
// This function formats the phone numbers correctly using regular expression
function formatPhoneNumber(phoneNumber) {
  //filters only numbers
  var cleaned = ('' + phoneNumber).replace(/\D/g, '');
  //checks correct lengths of numbers
  var match = cleaned.match(/^(1|)?(\d{3})(\d{3})(\d{4})$/);

  //Includes internation code if avaialable
  if (match) {
...
function makeMapItButton() {
  var mapContainer = document.createElement("div");
  var mapContainerIdAttr = document.createAttribute("id");
  mapContainerIdAttr.value = "map-div";
  mapContainer.setAttributeNode(mapContainerIdAttr);
  cardsEl.appendChild(mapContainer);
}
...
// Adding map it buttons after cards are created
document.body.addEventListener('click', function (event) {
  if (event.target.id == 'mapit-') {
    handleMapIt();
  };
});
// saved cities
